...
def _baixar_feed(url: str) -> bytes:
    """Baixa um feed com timeout, retries limitados e backoff."""
    ultima_excecao = None
    ...
    for tentativa in range(MAX_RETRIES + 1):
        try:
            ...
            if tentativa < MAX_RETRIES:
                espera = BACKOFF_SECONDS * (2**tentativa)
                log.warning(
                    f"Falha HTTP ao consultar '{url}'. "
    ...
    raise requests.RequestException(
        f"Falha após {MAX_RETRIES + 1} tentativas: {url}"
    ) from ultima_excecao
...
def buscar_rss_extra(url: str) -> list[dict]:
    ...
    try:
        ...
        for entry in feed.entries:
            ...
            if any(
                k in titulo + resumo
                for k in ["esfera", "transferência bonificada", "bonus transfer"]
            ):
                items.append(
                    {
                        "titulo": entry.get("title", ""),
